...
interface DoRescopeOfContentDao {
  ...
  @SqlUpdate(
      "delete from user_rescoped_content where user_id = any(:userIds) and course_id = :courseId and "
          +
          "class_id = :classId")
  ...
  void resetRescopeInfoInClassForAllUsers(@Bind("courseId") UUID courseId,
  ...
  @SqlUpdate(
      "delete from user_rescoped_content where user_id = any(:userIds) and course_id = :courseId and "
          +
          "class_id is null")
  void resetRescopeInfoForILForSpecifiedUsers(@Bind("userIds") PGArray<UUID> userIds,
