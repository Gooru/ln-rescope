 ...
 * <p>
 * Note that this class only handles the override flag, and if override is present it removes the rescope content
 * records for the specified users or for whole class based on type of rescope source. Then it delegates to downstream
 * for processsing. In case override is not present, it just delegates to downstream. Downstream processing services
 * check for rescope already done before they actually process the rescope request from the queue.
 *
...
class DoRescopeOfContentService {
    ...
    private void resetRescopeForSpecifiedMemersForIL() {
        fetchDao().resetRescopeInfoForILForSpecifiedUsers(
            CollectionUtils.convertFromListUUIDToSqlArrayOfUUID(command.getMemberIds()), command.getCourseId());
    }
